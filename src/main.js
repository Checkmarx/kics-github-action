...
function readFileContent(filePath) {
    try {
        // read file content
        const stats = fs.statSync( process.env.GITHUB_WOKSPACE+ "/"+filePath); // Use fs.statSync to get file stats synchronously
        if (!stats.isFile()) {
            throw new Error('Provided path is not a file.');
        }
        const data = fs.readFileSync(process.env.GITHUB_WOKSPACE+ "/"+filePath, 'utf8'); // Use fs.readFileSync to read file content synchronously
        return data;
    } catch (error) {
